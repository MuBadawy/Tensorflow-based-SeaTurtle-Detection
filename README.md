# How To use my trained sea turtle detection model or perform a custom object Detection model Using TensorFlow (GPU) on Windows 10

## Brief Summary
*Last updated: 17/06/2019 with TensorFlow v1.12*

This file describes how to use the already trained sea turtle detection model or even train your network for other objects:

1. [Installing TensorFlow-GPU](https://github.com/MuBadawy/Tensorflow-based-SeaTurtle-Detection/blob/master/Detection1/README.md#1-install-tensorflow-gpu-15-skip-this-step-if-tensorflow-gpu-15-is-already-installed)
...
## Steps
### 1. Install TensorFlow-GPU 1.12 (skip this step if TensorFlow-GPU 1.12 is already installed)
For installing TensorFlow-GPU, the instruction in this video will be useful [this YouTube Video by Mark Jay](https://www.youtube.com/watch?v=RplXYjxgZbw).

Make sure that you install Anaconda with Python 3.6 as instructed in the video, as the Anaconda virtual environment will be used for the rest of this tutorial.
...
Download every file in this repository and in the folder "detection1" as well. You will also need to download the training files and the inference graph form the links mentioned in the text files named training and that named inference graph. Alternatively you can download everything directly from this [google drive link](https://drive.google.com/open?id=1xN8loNggIATL6tGddK3KebYBhlmROOZS). Make sure that the downloaded folder is saved in your windows based drive (local disk C in most computers). Also make sure that the folder is named "tensorflow2" because there are some dependencies in the code on this path and this names. 
...
In the command terminal that pops up, create a new virtual environment called “tensorflow2” by issuing the following command:
```
C:\> conda create -n tensorflow2 pip python=3.6
```
...
```
(tensorflow2) C:\> pip install tensorflow-gpu==1.12
```
Install the other necessary packages by issuing the following commands:
```
(tensorflow2) C:\> conda install -c anaconda protobuf
(tensorflow2) C:\> pip install pillow
(tensorflow2) C:\> pip install lxml
(tensorflow2) C:\> pip install Cython
(tensorflow2) C:\> pip install jupyter
(tensorflow2) C:\> pip install matplotlib
(tensorflow2) C:\> pip install pandas
(tensorflow2) C:\> pip install opencv-python
```
...
```
(tensorflow2) C:\> set PYTHONPATH=C:\tensorflow2\models;C:\tensorflow2\models\research;C:\tensorflow2\models\research\slim
```
(Note: Every time the "tensorflow2" virtual environment is exited, the PYTHONPATH variable is reset and needs to be set up again.)
...
```
(tensorflow2) C:\tensorflow2\models\research\object_detection> jupyter notebook object_detection_tutorial.ipynb
```
...
```
(tensorflow2) C:\tensorflow2\models\research\object_detection> python sizeChecker.py --move
```
...
```
(tensorflow2) C:\tensorflow2\models\research\object_detection> python xml_to_csv.py
```
...
#### 5a. Label map
The label map tells the trainer what each object is by defining a mapping of class names to class ID numbers. Use a text editor to create a new file and save it as labelmap.pbtxt in the C:\tensorflow2\models\research\object_detection\training folder. (Make sure the file type is .pbtxt, not .txt !) In the text editor, copy or type in the label map in the format below (the example below is the label map for my Pinochle Deck Card Detector):
```
item {
  id: 1
  name: 'nine'
...
Navigate to C:\tensorflow2\models\research\object_detection\samples\configs and copy the faster_rcnn_inception_v2_pets.config file into the \object_detection\training directory. Then, open the file with a text editor. There are several changes to make to the .config file, mainly changing the number of classes and examples, and adding the file paths to the training data.
...
You can view the progress of the training job by using TensorBoard. To do this, open a new instance of Anaconda Prompt, activate the tensorflow2 virtual environment, change to the C:\tensorflow2\models\research\object_detection directory, and issue the following command:
```
...
To run any of the scripts, type “idle” in the Anaconda Command Prompt (with the “tensorflow2” virtual environment activated) and press ENTER. This will open IDLE, and from there, you can open any of the scripts and run them.
...
This error occurs when you try to run object_detection_tutorial.ipynb or train.py and you don’t have the PATH and PYTHONPATH environment variables set up correctly. Exit the virtual environment by closing and re-opening the Anaconda Prompt window. Then, issue “activate tensorflow2” to re-enter the environment, and then issue the commands given in Step 2e. 
